...
public class GameManager : MonoBehaviour
{
	...
	[SerializeField] float _scoreMult = 1;

	float _maxTime;
	float _decreaserMult = 1.49f;
	...
	{
		Score += (int)(_scoreMult);
		_scoreMult *= _decreaserMult;
		if (_decreaserMult > 1.1f)
			_decreaserMult *= 0.95f;

		_gameUI.MoveText((int)_scoreMult);
	}

	public void ResetMult()
	{
		_scoreMult = 1;
		_decreaserMult = 1.49f;
	}
